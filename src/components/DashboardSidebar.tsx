...
const JOBSEEKER_LINKS = [
  ...
  { href: "/tableau-de-bord/cv", label: "Mon CV", icon: FileEdit },
  ...
  return (
    ...
    <aside className="hidden md:flex flex-col w-56 bg-white border-r border-gray-100 min-h-[calc(100vh-4rem)] py-6 px-3 flex-shrink-0">
      ...
      <div className="flex items-center gap-3 px-3 mb-6">
        <div className="w-10 h-10 rounded-full bg-orange-100 flex items-center justify-center text-orange-600 font-bold text-sm flex-shrink-0 transition-transform duration-300 hover:scale-110 cursor-default relative">
          {initials}
      ...
      <nav className="flex-1 space-y-0.5">
        {links.map(({ href, label, icon: Icon, exact }) => {
          ...
          return (
            ...
            >
              {active && (
                <span className="sidebar-active-indicator absolute left-0 top-1/2 -translate-y-1/2 w-1 h-5 bg-orange-500 rounded-r-full" />
              )}
              <Icon className={`h-4 w-4 flex-shrink-0 transition-transform duration-200 group-hover:scale-110 ${active ? "text-orange-500" : ""}`} />
              {label}
            </Link>
          );
        })}
      ...
      <div className="mt-4 pt-4 border-t border-gray-100">
        <button
          onClick={() => setShowLogoutConfirm(true)}
          className="group flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-gray-500 hover:bg-red-50 hover:text-red-500 transition-all duration-200 w-full text-left"
        >
